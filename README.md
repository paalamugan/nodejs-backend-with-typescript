...
It is not a goal to be a comprehensive and definitive guide to making a TypeScript and Node project, but as a working reference maintained by the community. If you are interested in starting a new TypeScript project - check out the bootstrapping tools reference in [the TypeScript Website](https://www.typescriptlang.org/docs/home.html){:target="_blank"}
...
- [NODEJS BACKEND APPLICATION](#nodejs-backend-application)
...
```
git clone --depth=1 https://github.com/paalamugan/nodejs-backend-with-typescript.git <project_name>
```
...
If you haven't changed the name, this will be `NODEJS BACKEND WITH TYPESCRIPT`.
...
This will be part of the URL that azure generates, so it has to be unique, but if you're planning on adding a custom domain later, it's not that important. I usually just add random numbers to the end of the app name, ie. nodejs-backend-with-typescript-15121214.
...
Deployment can fail for various reasons, if you get stuck with a page that says *Service Unavailable* or some other error, [open an issue](https://github.com/paalamugan/nodejs-backend-with-typescript/issues/new) and I'll try to help you resolve the problems.
...
In this project, `bcrypt.d.ts`, `fbgraph.d.ts`, and `lusca.d.ts` were all generated using `dts-gen`.
...
A majority of this quick start's content was inspired or adapted from Sahat's [Hackathon Starter](https://github.com/sahat/hackathon-starter).
